...
export default function MessagesClient({ currentUserId, isBusiness, threads, payments, initialThreadId }) {
  ...
  const activeThread = threads.find((t) => t.id === activeId);
    ...
    const activePayment = (payments || []).find((p) => p.thread_id === activeId);
...
{activePayment && (
            <div className="px-4 py-3 border-b border-line bg-brand-soft flex items-center justify-between gap-3">
              <div>
                <div className="text-xs font-bold text-brand-dark uppercase tracking-wide">Sale Confirmed</div>
               <div className="text-xs text-muted">Confirmation #{activePayment.confirmation_number}</div>
  </div>
             <a
               href={`/api/sales/${activePayment.id}/document`}
              className="bg-brand text-white text-xs font-bold px-3.5 py-2 rounded-lg whitespace-nowrap"
            >
